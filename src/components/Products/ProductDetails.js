...
function ProductDetails() {
  ...
  productDetails.map(product => {
    let titleURL = '/' + product.title.replaceAll(' ', '%20')
    if(titleURL === window.location.pathname) {
      return (
        id = product.id,
        fullBrandName = product.brand.brandName + ' ' + product.brand.subBrand,
  ...
  return (
    <>
      ...
      {
        productDetails
        .filter(product => product.title !== title && product.brand.brandName + ' ' + product.brand.subBrand === fullBrandName).length > 0 ?
        <div className='moreLikeThis'>
        <div className='moreLikeThisTitle'>Similar Products</div>
        <div className='homeRow'>
          {
            productDetails && productDetails
            .filter(product => product.title !== title && product.brand.brandName + ' ' + product.brand.subBrand === fullBrandName)
            .map(productData => {
